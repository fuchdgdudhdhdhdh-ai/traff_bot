...
async def menu(target):
    """Показывает меню. Для CallbackQuery всегда удаляет предыдущий экран."""
    async with S() as s:
        title = await st(s, "text:menu", "🏠 <b>Главное меню</b>")
    markup = kb([("💰 Баланс","bal"),("👥 Рефералы","refs")],[("🔗 Моя ссылка","link"),("🆘 Поддержка","sup")],[("ℹ️ Информация","info"),("❓ Помощь","help")])
    if isinstance(target, CallbackQuery):
        return await replace_message(target, title, reply_markup=markup)
...
async def refs(c:CallbackQuery,bot:Bot):
 ...
 await replace_message(c, f"👥 Рефералов 1 уровня: {len(rows)}\n\n{names}",reply_markup=kb([("⬅️ Назад","home")]))
@r.callback_query(F.data=="link")
async def link(c:CallbackQuery,bot:Bot):
 if not await access(c,bot):return
 me=await bot.get_me();await replace_message(c, f"https://t.me/{me.username}?start=ref_{c.from_user.id}",reply_markup=kb([("⬅️ Назад","home")]))
...
async def sup(c:CallbackQuery,bot:Bot):
 ...
 await replace_message(c, "Напишите сообщение для поддержки.")
@r.message(F.photo)
...
async def atexts(c:CallbackQuery):
 ...
 await replace_message(c, "📝 <b>Редактирование текстов</b>\n\nВыберите экран, текст которого хотите изменить.", reply_markup=kb(
  ...
  [("ℹ️ Информация","a:text:info"),("❓ Помощь","a:text:help")],
...
async def atext_pick(c:CallbackQuery):
 ...
 if key not in {"menu","info","help"}:return
 async with S() as s:
...
async def admin_back(c:CallbackQuery):
 ...
 await replace_message(c, "🛠 <b>АДМИН-ПАНЕЛЬ</b>",reply_markup=kb([("👤 Пользователь","a:user"),("💸 Выплаты","a:payout")],[("🆘 Тикеты","a:tickets"),("🖼 Фото","a:photo")],
        [("⚙️ Каналы/ссылки","a:config"),("📝 Тексты","a:texts")],
        [("⚠️ Важные сообщения","a:notices"),("🔓 Открыть","a:open"),("🔒 Закрыть","a:close")]))
...
async def aph(c:CallbackQuery):
 ...
 await replace_message(c, "Отправьте фото с подписью menu, info или help.")
