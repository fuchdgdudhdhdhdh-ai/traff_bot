...
async def wdm(c:CallbackQuery,bot:Bot):
 ...
 await replace_message(c, f"Введите сумму к выводу (минимум 300 ₽).\n{hint}")
...
async def on_text(m:Message):
 async with S() as s:
  ...
  if x and x.action=="wd_amount":
   ...
   if a<300:await m.answer("Минимальная сумма вывода — 300 ₽.");return
   method=x.data
...
async def admin(m:Message):
 ...
 await m.answer("🛠 <b>АДМИН-ПАНЕЛЬ</b>",reply_markup=kb([("📊 Статистика","a:stats"),("👤 Пользователь","a:user")],[("👥 Все пользователи","a:users:all:0")],[("💸 Выплаты","a:payout"),("🆘 Тикеты","a:tickets")],[("📢 Рассылка","a:broadcast"),("🖼 Фото","a:photo")],[("⚙️ Каналы/ссылки","a:config")],
        [("📝 Тексты","a:texts")],
...
async def admin_back(c:CallbackQuery):
 ...
 await replace_message(c, "🛠 <b>АДМИН-ПАНЕЛЬ</b>",reply_markup=kb([("📊 Статистика","a:stats"),("👤 Пользователь","a:user")],[("👥 Все пользователи","a:users:all:0")],[("💸 Выплаты","a:payout"),("🆘 Тикеты","a:tickets")],[("📢 Рассылка","a:broadcast"),("🖼 Фото","a:photo")],[("⚙️ Каналы/ссылки","a:config")],
        [("📝 Тексты","a:texts")],
        [("⚠️ Важные сообщения","a:notices"),("🔓 Открыть выплаты","a:open"),("🔒 Закрыть выплаты","a:close")],
        [("🛑 Закрыть бота","a:botclose"),("▶️ Открыть бота","a:botopen_menu")]))
