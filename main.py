...
async def access(c,bot):
 ...
 if not u or u.blocked or not u.captcha_ok or not await sub(bot,c.from_user.id):
  ...
  await replace_message(c, "⚠️ Нет доступа. Подпишитесь на каналы и нажмите «Я подписался».", reply_markup=markup)
  return False
 return True
