...
async def required_channels_ok(bot, uid):
    # Проверяем ТОЛЬКО подписку на публичный канал.
    # Все приватные каналы полностью игнорируются: бот не проверяет
    # ни заявку, ни членство в них.
    async with S() as s:
        public = await public_channel(s)
    return await channel_member(bot, public, uid) if public else True
...
async def wd(c:CallbackQuery,bot:Bot):
 if not await access(c,bot):return
 async with S() as s:
  manual=await st(s,"payout_manual","auto");open_=manual=="open" or (manual=="auto" and datetime.now(timezone.utc).weekday() in (2,6))
 if not open_:await c.answer("💸 Выплаты производятся от 500 ₽, по средам и воскресеньям. Попробуйте в один из этих дней.",show_alert=True);return
 await replace_message(c, "Способ:",reply_markup=kb([("💳 Карта","wd:card"),("📱 Телефон","wd:phone")],[("₮ USDT","wd:usdt"),("🎫 Чек","wd:check")]))
...
async def wdm(c:CallbackQuery,bot:Bot):
 ...
 await replace_message(c, "Введите сумму (минимум 500).")
...
async def atext_pick(c:CallbackQuery):
 ...
 if key not in {"menu","info"}:return
 async with S() as s:
...
async def all_users(c:CallbackQuery):
 ...
 async with S() as s:
  ...
  rows=(await s.scalars(select(User).order_by(User.created_at.desc()).offset(offset).limit(USERS_PAGE_SIZE))).all()
  lines=[]
  for u in rows:
   ref_label="—"
   if u.referrer_id:
    ref=await s.get(User,u.referrer_id)
    if ref: ref_label=f"@{ref.username}" if ref.username else f"ID {ref.id}"
    else: ref_label=f"ID {u.referrer_id} (удалён)"
   uname=f"@{u.username}" if u.username else f"ID {u.id}"
   flag=" ⛔" if u.blocked else ""
   lines.append(f"• {uname}{flag} (ID <code>{u.id}</code>)\n  Пригласил: {ref_label}\n  Баланс: {u.balance} ₽")
 body="\n\n".join(lines) if lines else "Пользователей нет."
 header=f"👥 <b>Все пользователи</b>\nВсего: {total}, показаны {offset+1}-{offset+len(rows)}\n\n"
 nav=[]
 if offset>0: nav.append(("⬅️ Пред.",f"a:users:{max(0,offset-USERS_PAGE_SIZE)}"))
 if offset+USERS_PAGE_SIZE<total: nav.append(("➡️ След.",f"a:users:{offset+USERS_PAGE_SIZE}"))
 rows_kb=([nav] if nav else [])+[[("🔙 Админ-панель","a:admin")]]
 await replace_message(c, header+body, reply_markup=kb(*rows_kb))
...
async def block(c:CallbackQuery):
 ...
 await replace_message(c, "Статус изменён.")
...
async def ats(c:CallbackQuery):
 ...
 if not rows:
  ...
  return
 lines="\n".join(f"• #{t.id} — {t.user_id}" for t in rows[:50])
 await replace_message(c,"🆘 <b>Открытые тикеты</b>\n"+lines,reply_markup=kb(*[[(f"Тикет #{t.id}",f"a:ticket:{t.id}")] for t in rows[:20]]+[["🔙 Админ-панель","a:admin"]]))
...
async def ap(c:CallbackQuery):
 ...
 if not rows:
  ...
  return
 await replace_message(c,"💸 <b>Заявки на выплату</b>",reply_markup=kb(*[[(f"#{w.id} — {w.amount}",f"a:payview:{w.id}")] for w in rows[:20]]+[["🔙 Админ-панель","a:admin"]]))
...
async def aph(c:CallbackQuery):
 ...
 await replace_message(c, "Отправьте фото с подписью: menu, info, refs, link, sup или bal.")
