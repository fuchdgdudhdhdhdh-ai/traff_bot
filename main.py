...
async def join_kb(s):
 ...
 return kb(*rows)
def kb(*rows):return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=x,callback_data=y) for x,y in row] for row in rows])
async def st(s,k,d=""):
...
async def access(c,bot):
 ...
 if not u or u.blocked or not u.captcha_ok or not await sub(bot,c.from_user.id):
  await c.message.answer("⚠️ Нет доступа. Подпишитесь на публичный канал.",reply_markup=kb([("📢 Канал",PUBURL)],[("🔄 Проверить","check")]))
  return False
 return True
async def menu(m):
...
async def cap(c:CallbackQuery):
 ...
 await c.message.answer("Подпишитесь на каналы.",reply_markup=kb([("🔒 Частный",PRIVURL)],[("📢 Публичный",PUBURL)],[("✅ Я подписался","check")]))
