...
async def access(c,bot):
    ...
    if closed:
        data=c.data or ""
        if data not in BOT_OPEN_ALLOWED_DATA and not data.startswith("wd:"):
            await c.answer("🛑 Бот временно приостановлен администрацией.\nДоступны только: Баланс, Рефералы, Выплаты.",show_alert=True)
            return False
    return True
...
async def on_text(m:Message):
 async with S() as s:
  ...
  if x and x.action=="wd_dest":
   method,a=x.data.split("|");a=Decimal(a);fee=FEES.get(method,Decimal(0));net=a-fee
...
async def admin(m:Message):
 ...
 await m.answer("🛠 <b>АДМИН-ПАНЕЛЬ</b>",reply_markup=kb([("📊 Статистика","a:stats"),("👤 Пользователь","a:user")],[("👥 Все пользователи","a:users:0")],[("💸 Выплаты","a:payout"),("🆘 Тикеты","a:tickets")],[("📢 Рассылка","a:broadcast"),("🖼 Фото","a:photo")],[("⚙️ Каналы/ссылки","a:config")],
        [("📝 Тексты","a:texts")],
...
async def atext_pick(c:CallbackQuery):
 ...
 if key not in {"menu","info","faq"}:return
 async with S() as s:
...
async def admin_back(c:CallbackQuery):
 ...
 await replace_message(c, "🛠 <b>АДМИН-ПАНЕЛЬ</b>",reply_markup=kb([("📊 Статистика","a:stats"),("👤 Пользователь","a:user")],[("👥 Все пользователи","a:users:0")],[("💸 Выплаты","a:payout"),("🆘 Тикеты","a:tickets")],[("📢 Рассылка","a:broadcast"),("🖼 Фото","a:photo")],[("⚙️ Каналы/ссылки","a:config")],
        [("📝 Тексты","a:texts")],
        [("⚠️ Важные сообщения","a:notices"),("🔓 Открыть выплаты","a:open"),("🔒 Закрыть выплаты","a:close")],
        [("🛑 Закрыть бота","a:botclose"),("▶️ Открыть бота","a:botopen_menu")]))
...
async def bot_close(c:CallbackQuery):
 ...
 await replace_message(c, "🛑 Бот закрыт.\n\nОбычные пользователи теперь видят ограниченное меню: Баланс, Рефералы, Выплаты. Новые пользователи (кто ещё не заходил в бота) при старте получают сообщение о временной приостановке работы.", reply_markup=kb([("🔙 Админ-панель","a:admin")]))
