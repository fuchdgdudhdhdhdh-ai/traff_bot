...
class User(B):
 __tablename__="users";id:Mapped[int]=mapped_column(Integer,primary_key=True);username:Mapped[str|None]=mapped_column(String(64),nullable=True);referrer_id:Mapped[int|None]=mapped_column(Integer,nullable=True);balance:Mapped[Decimal]=mapped_column(Numeric(12,2),default=0);captcha_ok:Mapped[bool]=mapped_column(Boolean,default=False);verified:Mapped[bool]=mapped_column(Boolean,default=False);blocked:Mapped[bool]=mapped_column(Boolean,default=False);created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=lambda:datetime.now(timezone.utc))
class Admin(B):__tablename__="admins";user_id:Mapped[int]=mapped_column(Integer,primary_key=True);role:Mapped[str]=mapped_column(String(20))
class Reward(B):__tablename__="rewards";id:Mapped[int]=mapped_column(Integer,primary_key=True);referrer_id:Mapped[int]=mapped_column(Integer);referral_id:Mapped[int]=mapped_column(Integer,unique=True);amount:Mapped[Decimal]=mapped_column(Numeric(12,2));status:Mapped[str]=mapped_column(String(20));hold_until:Mapped[datetime]=mapped_column(DateTime(timezone=True));last_ok:Mapped[bool]=mapped_column(Boolean,default=True)
class Ledger(B):__tablename__="ledger";id:Mapped[int]=mapped_column(Integer,primary_key=True);user_id:Mapped[int]=mapped_column(Integer);amount:Mapped[Decimal]=mapped_column(Numeric(12,2));kind:Mapped[str]=mapped_column(String(40));description:Mapped[str]=mapped_column(Text);created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=lambda:datetime.now(timezone.utc))
class Withdrawal(B):__tablename__="withdrawals";id:Mapped[int]=mapped_column(Integer,primary_key=True);user_id:Mapped[int]=mapped_column(Integer);amount:Mapped[Decimal]=mapped_column(Numeric(12,2));method:Mapped[str]=mapped_column(String(40));destination:Mapped[str]=mapped_column(Text);status:Mapped[str]=mapped_column(String(20),default="pending");created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=lambda:datetime.now(timezone.utc))
class Ticket(B):__tablename__="tickets";id:Mapped[int]=mapped_column(Integer,primary_key=True);user_id:Mapped[int]=mapped_column(Integer);status:Mapped[str]=mapped_column(String(20),default="open");created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=lambda:datetime.now(timezone.utc))
class TM(B):__tablename__="ticket_messages";id:Mapped[int]=mapped_column(Integer,primary_key=True);ticket_id:Mapped[int]=mapped_column(Integer);sender_id:Mapped[int]=mapped_column(Integer);text:Mapped[str]=mapped_column(Text);created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=lambda:datetime.now(timezone.utc))
...
class State(B):__tablename__="states";user_id:Mapped[int]=mapped_column(Integer,primary_key=True);action:Mapped[str]=mapped_column(String(50),default="");data:Mapped[str]=mapped_column(Text,default="")
class Notice(B):
...
async def send_screen(c: CallbackQuery, text: str, media_key: str | None = None, **kwargs):
    ...
    if media_key:
        async with S() as s: pic = await s.get(Media, "photo:" + media_key)
        if pic:
...
async def menu(target):
    ...
    if isinstance(target, CallbackQuery):
        ...
        if pic: return await target.bot.send_photo(target.message.chat.id,pic.file_id,caption=title,reply_markup=markup)
        return await target.bot.send_message(target.message.chat.id,title,reply_markup=markup)
    if pic: return await target.answer_photo(pic.file_id,caption=title,reply_markup=markup)
...
async def refs(c:CallbackQuery,bot:Bot):
 ...
 await send_screen(c, f"👥 Рефералы: {len(rows)}\n\n{names}", media_key="refs", reply_markup=kb([("⬅️ Назад","home")]))
@r.callback_query(F.data=="link")
async def link(c:CallbackQuery,bot:Bot):
 if not await access(c,bot):return
 me=await bot.get_me();await send_screen(c, f"https://t.me/{me.username}?start=ref_{c.from_user.id}", media_key="link", reply_markup=kb([("⬅️ Назад","home")]))
...
async def admin_refs(c:CallbackQuery):
 ...
 await c.message.answer(f"👥 Рефералы пользователя {uid}\\nКоличество: {len(rows)}\\n\\n{text}")
...
async def aph(c:CallbackQuery):
 ...
 await c.message.answer("Отправьте фото с подписью: menu, info, refs, link, sup или bal.")
@r.callback_query(F.data=="info")
async def content(c:CallbackQuery,bot:Bot):
 ...
 await send_screen(c, txt, media_key=c.data, reply_markup=kb([("⬅️ Назад","home")]))
